...
def fit_and_draw_ellipse(image, contour_goutte):
    ...
    return theta, xc, yc, x, y, a, b, angle
...
def draw_point(x,y,image):
    ...
    couleur = (0, 255, 0)  # Vert
            ...
            # Dessiner le point
    cv2.circle(image, (x, y), 3, couleur, 2)
    return 0
...
def main(image_path):
    ...
    theta, xc, yc, x, y, a, b, angle_ellipse = fit_and_draw_ellipse(image_original, contour_goutte)
    
    ellipse_params = (a, b, xc, yc)  
    
    #L'ordonnée du point ou il faut tracer la tangente
    y_base = base[0][1]
    #Les abscisses qui correspondent à l'odonnée sur l'ellipse
    x1 = xc + 0.5*b*math.sqrt(1 - ((y_base - yc) / (0.5*a))**2)
    x2 = xc - 0.5*b* math.sqrt(1 - ((y_base - yc) / (0.5*a))**2)

    #Calcul de l'angle entre l'horizontale et la droite qui passe par le centre de l'ellipse et le point ou on trace la tangente
    m = (-y_base + yc)/(x1 - xc)
    angle_int = np.arctan((m))
    angle_deg = math.degrees(angle_int)

    #
    angle=(angle_int*1000)/(2*math.pi)
    ...
    #Tenir compte du déphasage de l'ellipse dans le repère
    indice = int(indice_plus_proche + angle_ellipse)
    
    #Coordonée du point ou on trace la tangente
    xt=x1
    yt=y_base
    
    
    draw_tangent_on_image(image_original,a,b,x,y,xt,yt,indice)
...
if __name__ == "__main__":
    image_path = "D:\PycharmDossier\AdlerGoutteEau\image_goutte12.png"
